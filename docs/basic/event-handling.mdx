...
:::tip
在 [`mirai.models.events`](https://yiri-mirai-api.vercel.app/models/events.html) 模块中找到更多事件类型。
:::
...
你可以从 [`mirai.models.events`](https://yiri-mirai-api.vercel.app/models/events.html) 模块引入这些事件。部分常用的事件也可从顶层模块 `mirai` 的命名空间引入。

## 传播中止

如果想要阻止事件被其他事件处理器监听，或者想要阻止事件向上一级事件链传播，可以采用传播中止的方式。

YiriMirai 通过抛出异常实现传播中止。在 [`mirai.exceptions`](https://yiri-mirai-api.vercel.app/exceptions.html) 模块中，定义了以下几个异常：
